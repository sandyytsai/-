...
def load_results(hourly_csv="results_optimization_15m.csv", pumps_csv="results_pumps_15m.csv"):
...
def plot_system_overview(hourly, Vmin=None, Vmax=None, show_export=True):
    """
    Single figure with 3 stacked subplots:
      1) Pond volume (with Vmin/Vmax)
      2) Pumped volume per step
      3) Energy allocation: PV->pumps, Grid->pumps (+ optional export) and PV available
    """
    _require_cols(hourly, ["hour", "V_m3", "Vpump_m3", "e_pv_kwh", "e_g_kwh", "pv_total_kwh"], name="hourly")
    if show_export:
        _require_cols(hourly, ["export_kwh"], name="hourly")

    x = hourly["hour"].to_numpy()

    V = hourly["V_m3"].to_numpy()
    Vpump = hourly["Vpump_m3"].to_numpy()
    ...
    # --- (1) Pond volume ---
    ax = axes[0]
    ax.plot(x, V, linewidth=2, label="Pond volume (m³)")
    if Vmin is not None:
        ax.axhline(Vmin, linestyle="--", label="Vmin")
    if Vmax is not None:
        ax.axhline(Vmax, linestyle="--", label="Vmax")
    ax.set_ylabel("m³")
    ax.legend(loc="best")

    # --- (2) Pumping ---
    ax = axes[1]
    ax.bar(x, Vpump, label="Pumped volume (m³/15min)")
    ax.set_ylabel("m³ / 15min")
    ax.legend(loc="best")
    ...
    if show_export:
        export = hourly["export_kwh"].to_numpy()
        ax.stackplot(x, e_pv, export, e_g, labels=["PV → pumps", "PV export", "Grid → pumps"])
    else:
        ax.stackplot(x, e_pv, e_g, labels=["PV → pumps", "Grid → pumps"])

    ax.plot(x, pv_total, linewidth=2, label="PV available (kWh/15min)")
    ax.set_ylabel("kWh / 15min")
    ax.set_xlabel("Step index (15-min index)")
    ax.legend(loc="upper right")

    fig.suptitle("System overview: pond level, pumping, and power allocation")
    fig.tight_layout()


# One-call convenience: generate the “core” set of charts

def plot_all(hourly_csv="results_optimization_15m.csv", pumps_csv="results_pumps_15m.csv",
    ...
    plot_system_overview(hourly, Vmin=Vmin, Vmax=Vmax, show_export=True)
